...
src/
  ...
  components/
    layout/               # Header (client, mobile menu + language/theme toggles),
                          #   RouteTransition (client route-mount opacity fade), Footer
...
- Keep components small and composable; share spacing via `Container` and headings via
  ...
  standalone page. Use the `Button` component for all CTAs (it renders a plain `<a>` for
  absolute external URLs, `<Link>` for internal routes, or `<button>` for actions).
...
- Route changes get a short mount-only opacity fade through `app/template.tsx` and
  `components/layout/RouteTransition.tsx`. Keep it subtle, avoid vertical movement on body
  content, and preserve the `prefers-reduced-motion` override in `globals.css`. The sticky
  header and footer stay outside this transition.
